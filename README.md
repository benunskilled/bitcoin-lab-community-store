...
[`bitcoin-lab`](https://github.com/benunskilled/bitcoin-lab) repository.
...
1. **Create the `bitcoin-lab` source repo** on GitHub under `benunskilled`
   (public, so the image build and Umbrel's image pull can both reach it)
   and push the contents of the sibling `bitcoin-lab/` folder to it.
2. **Tag a release** to trigger the multi-arch image build:
   ...
   `ghcr.io/benunskilled/bitcoin-lab:1.0.0@sha256:<digest>` reference.
3. **Paste that reference** into all four `image:` lines in
   `bitcoinlab-node/docker-compose.yml` (replacing
   `sha256:REPLACE_AFTER_FIRST_RELEASE`), commit, push.
4. **Make the GHCR image public.** By default a package pushed by CI is
   private to your account - open
   `https://github.com/users/benunskilled/packages/container/bitcoin-lab/settings`
   and set visibility to Public, otherwise Umbrel cannot pull it.
5. **Push this repo** (`bitcoin-lab-community-store`) to GitHub as well,
   under `benunskilled`.
6. In the Umbrel UI: **Settings → App Store → Community App Stores → Add
   App Store**, paste `https://github.com/benunskilled/bitcoin-lab-community-store`,
   then install "Bitcoin Lab" from the "Bitcoin Lab" store like any other
   app. It will prompt you to install the official **Bitcoin Node** app
   first if you haven't already (declared as a `dependencies:`
   requirement).
